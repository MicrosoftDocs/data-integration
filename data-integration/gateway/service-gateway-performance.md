...
|**EvaluationContext**|Contains the artifactId (eg., ModelID, DataflowsId, etc.) along with additional trace Id data that varies depending on the artifact. Note that this field only populates for Semantic Models, Dataflow Gen2, Power Platform dataflows, and Azure Analysis Services. Power BI Dataflows Gen1 and Paginated Reports are not supported. |
