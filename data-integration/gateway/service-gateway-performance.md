...
|**EvaluationContext**|Contains the artifactId (ie., datasetid for semantic models, dataflowsId, etc.) along with additional trace ids depending on the artifact. Note that this field only populates for supported workloads in Fabric, Power Platform, Azure Analysis Services, and certain connectors in Azure Logic Apps. These include: Semantic Models, Dataflow Gen2, Power Platform dataflows, Power Automate flows, the Data Factory Pipelines Dataflow activity, and Azure Analysis Services. It also includes Azure Logic Apps workloads for the Mashup Query, HTTP, and File Azure connectors. Power BI and Fabric Dataflows Gen 1 and Power BI Paginated Reports is not supported. |
