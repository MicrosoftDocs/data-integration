...
> [!NOTE]
> The EvaluationContext will show up without additional logging being enabled for Power BI Dataflows and Dataflows Gen2 for users on Pro licenses only. You still need additional logging enabled to view EvaluationContext for Power BI Dataflows for users on Premium licenses.
...
|**EvaluationContext**|Contains the artifactId (ie. DatasetId, DataflowsId) along with additional data depending on the artifact.|
