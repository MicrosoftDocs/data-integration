...
* Using an on-premises data gateway with private link will result in traffic that does not travel through the private link. For this reason, it is not a reccomended nor supported  scenario. However, if you would like to go ahead and use it anyway, it is possible. When private link is enabled, disable private link before installing the gateway. After installation, you can re-enable it. If private link is enabled, you'll get the following error when trying to register a new gateway or migrate/restore/takeover an existing gateway:
