...
We recommend that you allow the DNS "\*.servicebus.windows.net". For guidance on how to set up your on-prem firewall and/or proxy using FQDNs, instead of using IP addresses that are subject to change, follow the steps in [Azure WCF Relay DNS Support](https://techcommunity.microsoft.com/t5/messaging-on-azure/azure-wcf-relay-dns-support/ba-p/370775).
...
The gateway communicates with Service Bus by using fully qualified domain names (FQDNs). If you force the gateway to communicate via HTTPS, it will strictly use FQDNs only and won't communicate by using IP addresses.
